...
server.tool(
  ...
  'Send an image to the user or group. The image must be a file within /workspace/group/ (e.g., /workspace/group/images/chart.png). Optionally include a caption.',
  {
    image_path: z
      .string()
      .describe(
        'Path to the image file within /workspace/group/ (e.g., "/workspace/group/images/chart.png" or "images/chart.png")',
      ),
    caption: z
      .string()
      .optional()
    ...
    sender: z
      ...
      .optional()
      .describe(
        'Your role/identity name (e.g. "Researcher"). When set, images appear from a dedicated bot in Telegram.',
      ),
  },
  async (args) => {
        ...
        content: [
          {
            type: 'text' as const,
            text:
              'Image path must be within /workspace/group/. Got: ' +
              args.image_path,
          },
        ],
        isError: true,
    ...
    if (!fs.existsSync(fullPath)) {
      return {
        content: [
          { type: 'text' as const, text: 'Image file not found: ' + fullPath },
        ],
        isError: true,
      };
    }

    // Defense in depth: refuse symlinks or paths that resolve outside
    // /workspace/group (host also re-checks with realpath).
    const groupRoot = fs.realpathSync('/workspace/group');
    const realResolved = fs.realpathSync(fullPath);
    if (!realResolved.startsWith(groupRoot + path.sep)) {
      return {
        content: [
          {
            type: 'text' as const,
            text: 'Image path resolves outside /workspace/group via symlink: ' + args.image_path,
          },
        ],
        isError: true,
      };
